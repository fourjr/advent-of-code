...
painting = [Location(0, 0, Colors.WHITE)]
...
multiplier = 30
size = (((abs(min(x_coords)) + max(x_coords) + 1) * multiplier), (abs(min(y_coords)) + max(y_coords) + 1) * multiplier)
...
for i in painting:
    if i.color == Colors.WHITE:
        fill = (255, 255, 255)
    elif i.color == Colors.BLACK:
        fill = (0, 0, 0)

    x1 = (i.x - min(x_coords)) * multiplier
    y1 = (i.y - min(y_coords)) * multiplier
    draw.rectangle((x1, y1, x1 + multiplier - 1, y1 + multiplier - 1), fill=fill)
